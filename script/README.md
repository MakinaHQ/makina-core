...
1. Copy `script/deploy/inputs/hub-cores/TEMPLATE.json` to `script/deploy/inputs/hub-cores/{HUB_CORE_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/hub-cores/{HUB_CORE_OUTPUT_FILENAME}` containing the deployed contract addresses.

```shell
forge script script/deploy/DeployHubCore.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```
...
1. Copy `script/deploy/inputs/hub-cores/Foreign-TEMPLATE.json` to `script/deploy/inputs/hub-cores/{HUB_CORE_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/hub-cores/{HUB_CORE_OUTPUT_FILENAME}` containing the addresses of the contracts deployed by this script only.

```shell
forge script script/deploy/DeployForeignHubCore.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```
...
```shell
VIEW_MODE=true forge script script/deploy/SetupForeignHubCore.s.sol --rpc-url <network-alias> -vvvv
```
...
1. Copy `script/deploy/inputs/hub-machines/TEMPLATE.json` to `script/deploy/inputs/hub-machines/{HUB_STRAT_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/hub-machines/{HUB_STRAT_OUTPUT_FILENAME}`.

```shell
forge script script/deploy/DeployHubMachine.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```

#### Pre-Deposit Vault instance

1. Copy `script/deploy/inputs/pre-deposit-vaults/TEMPLATE.json` to `script/deploy/inputs/pre-deposit-vaults/{HUB_STRAT_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/pre-deposit-vaults/{HUB_STRAT_OUTPUT_FILENAME}`.

```shell
forge script script/deploy/DeployPreDepositVault.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```

#### Pre-Deposit Vault instance migration into Hub Machine instance

1. Copy `script/deploy/inputs/pre-deposit-migrations/TEMPLATE.json` to `script/deploy/inputs/pre-deposit-migrations/{HUB_STRAT_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/pre-deposit-migrations/{HUB_STRAT_OUTPUT_FILENAME}`.

```shell
forge script script/deploy/DeployHubMachineFromPreDeposit.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```
...
1. Copy `script/deploy/inputs/spoke-cores/TEMPLATE.json` to `script/deploy/inputs/spoke-cores/{SPOKE_CORE_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/spoke-cores/{SPOKE_CORE_OUTPUT_FILENAME}`.

```shell
forge script script/deploy/DeploySpokeCore.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```
...
1. Copy `script/deploy/inputs/spoke-cores/Foreign-TEMPLATE.json` to `script/deploy/inputs/spoke-cores/{SPOKE_CORE_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/spoke-cores/{SPOKE_CORE_OUTPUT_FILENAME}` containing the addresses of the contracts deployed by this script only.

```shell
forge script script/deploy/DeployForeignSpokeCore.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```

3. Run the following command to log the calldata wiring the deployed contracts, as described for the hub chain.

```shell
VIEW_MODE=true forge script script/deploy/SetupForeignSpokeCore.s.sol --rpc-url <network-alias> -vvvv
```
...
1. Copy `script/deploy/inputs/spoke-calibers/TEMPLATE.json` to `script/deploy/inputs/spoke-calibers/{SPOKE_STRAT_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/spoke-calibers/{SPOKE_STRAT_OUTPUT_FILENAME}`.

```shell
forge script script/deploy/DeploySpokeCaliber.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```
...
1. Copy `script/deploy/inputs/timelock-controllers/TEMPLATE.json` to `script/deploy/inputs/timelock-controllers/{TIMELOCK_CONTROLLER_INPUT_FILENAME}` and fill in the required variables.
2. Run the following command to initiate the deployment. This will generate an output file at `script/deploy/outputs/timelock-controllers/{TIMELOCK_CONTROLLER_OUTPUT_FILENAME}`.

```shell
forge script script/deploy/DeployTimelockController.s.sol --rpc-url <network-alias> <wallet-options> --slow --broadcast --verify -vvvv
```
