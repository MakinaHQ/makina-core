...
contract Machine is MakinaGovernable, BridgeController, ReentrancyGuard, IMachine {
    ...
    function transferToHubCaliber(address token, uint256 amount) external override notRecoveryMode onlyMechanic {
        ...
        IERC20Metadata(token).forceApprove($._hubCaliber, amount);
    ...
    function transferToSpokeCaliber(
        ...
        if (caliberData.disabled) {
            revert Errors.SpokeCaliberNotEnabled();
        }
    ...
    function disableSpokeCaliber(uint256 chainId) external override nonReentrant restricted {
        ...
        if (caliberData.netAum != 0) {
            revert Errors.CaliberNotEmpty();
        }

        MachineUtils.checkBridgeTransfersSettled(caliberData);
