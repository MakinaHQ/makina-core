...
contract Deposit_Integration_Concrete_Test is Machine_Integration_Concrete_Test {
    function test_RevertWhen_ReentrantCall() public {
        accountingToken.scheduleReenter(
            MockERC20.Type.Before, address(machine), abi.encodeCall(IMachine.deposit, (0, address(0), 0, 0))
        );
    ...
    function test_RevertGiven_WhileInRecoveryMode() public whileInRecoveryMode {
        ...
        machine.deposit(1e18, address(this), 0, 0);
    }
    ...
    function test_RevertWhen_SlippageProtectionTriggered() public {
        ...
        vm.prank(machineDepositor);
        machine.deposit(inputAmount, address(this), expectedShares + 1, 0);
    }
    ...
    function test_Deposit() public {
        ...
        emit IMachine.Deposit(machineDepositor, address(this), inputAmount, expectedShares, 0);
        machine.deposit(inputAmount, address(this), expectedShares, 0);
    ...
    function test_Deposit_WithReferral() public {
        ...
        accountingToken.approve(address(machine), inputAmount);
        vm.expectEmit(true, true, false, true, address(machine));
        emit IMachine.Deposit(machineDepositor, address(this), inputAmount, expectedShares, referralKey);
        machine.deposit(inputAmount, address(this), expectedShares, referralKey);
