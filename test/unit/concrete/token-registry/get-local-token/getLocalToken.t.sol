...
contract GetLocalToken_Unit_Concrete_Test is TokenRegistry_Unit_Concrete_Test {
    function test_RevertWhen_TokenNotRegistered() public {
        vm.expectRevert(abi.encodeWithSelector(ITokenRegistry.LocalTokenNotRegistered.selector, address(0), 0));
        tokenRegistry.getLocalToken(address(0), 0);

        vm.expectRevert(abi.encodeWithSelector(ITokenRegistry.LocalTokenNotRegistered.selector, address(2), 2));
        tokenRegistry.getLocalToken(address(2), 2);

        vm.expectRevert(abi.encodeWithSelector(ITokenRegistry.LocalTokenNotRegistered.selector, address(1), 1));
        ...
        vm.expectRevert(abi.encodeWithSelector(ITokenRegistry.LocalTokenNotRegistered.selector, address(1), 2));
